...
void RunProgram_Reduced(int id, int np, std::string InputFile) {
    ...
    // Set up domain decomposition and allocate ghosted arrays for gathers.
    Halo halo(id, np, nx, ny, nz);
    // Local copies of halo class details.
    auto MyYSlices = halo.local_y;
    auto MyYOffset = halo.offset_y;
    auto LocalDomainSize = halo.local_xyz;
    auto AtNorthBoundary = halo.boundary_north;
    auto AtSouthBoundary = halo.boundary_south;
    ...
    if (SimulationType == "C") {
        SubstrateInit_ConstrainedGrowth(FractSurfaceSitesActive, NeighborX, NeighborY, NeighborZ, GrainUnitVector,
                                        NGrainOrientations, CellType, GrainID, DiagonalLength, DOCenter,
                                        CritDiagonalLength, RNGSeed, halo);
    }
    else {
        if (UseSubstrateFile)
        ...
        if (RemeltingYN)
            ...
            CellTypeInit_NoRemelt(0, halo, ZBound_Low, LocalActiveDomainSize, CellType, CritTimeStep, NeighborX,
    ...
    if ((np > 1) && (!(RemeltingYN))) {
        halo.gather(NeighborX, NeighborY, NeighborZ, CellType, DOCenter, GrainID, GrainUnitVector, DiagonalLength,
                    CritDiagonalLength, NGrainOrientations, ZBound_Low);
    }
    ...
    if (PrintDebug) {
        ...
        // subroutine
        PrintExaCAData(-1, halo, GrainID, CritTimeStep, GrainUnitVector, LayerID, CellType, UndercoolingChange,
                       UndercoolingCurrent, OutputFile, NGrainOrientations, PathToOutput, PrintDebug, false, false,
                       false, false, false, 0, ZBound_Low, nzActive, deltax, XMin, YMin, ZMin, NumberOfLayers,
                       PrintBinary);
        MPI_Barrier(MPI_COMM_WORLD);
        if (id == 0)
    ...
    for (int layernumber = 0; layernumber < NumberOfLayers; layernumber++) {
        ...
        do {
            ...
            if ((PrintTimeSeries) && (cycle % TimeSeriesInc == 0)) {
                ...
                PrintExaCAData(layernumber, halo, GrainID, CritTimeStep, GrainUnitVector, LayerID, CellType,
                               UndercoolingChange, UndercoolingCurrent, OutputFile, NGrainOrientations, PathToOutput, 0,
                               false, false, false, true, false, IntermediateFileCounter, ZBound_Low, nzActive, deltax,
                               XMin, YMin, ZMin, NumberOfLayers, PrintBinary);
            ...
            if (RemeltingYN)
                FillSteeringVector_Remelt(cycle, halo, LocalActiveDomainSize, NeighborX, NeighborY, NeighborZ,
            ...
            CellCapture(halo, irf, NeighborX, NeighborY, NeighborZ, CritTimeStep, UndercoolingCurrent,
                        UndercoolingChange, GrainUnitVector, CritDiagonalLength, DiagonalLength, CellType, DOCenter,
                        GrainID, NGrainOrientations, ZBound_Low, nzActive, SteeringVector, numSteer, numSteer_Host,
            ...
            if (np > 1) {
                ...
                halo.gather(NeighborX, NeighborY, NeighborZ, CellType, DOCenter, GrainID, GrainUnitVector,
                            DiagonalLength, CritDiagonalLength, NGrainOrientations, ZBound_Low);
            ...
            if (cycle % 1000 == 0) {
                ...
                if (RemeltingYN)
                    IntermediateOutputAndCheck_Remelt(
                        cycle, halo, LocalActiveDomainSize, nzActive, deltax, XMin, YMin, ZMin,
                        SuccessfulNucEvents_ThisRank, XSwitch, CellType, CritTimeStep, GrainID, SimulationType,
                    ...
                    IntermediateOutputAndCheck(cycle, halo, LocalActiveDomainSize, nzActive, deltax, XMin, YMin, ZMin,
        ...
        if (layernumber != NumberOfLayers - 1) {
            ...
            MPI_Barrier(MPI_COMM_WORLD);
            if (id == 0)
                std::cout << "Resize executed and new layer set up, GN dimensions are " << halo.buffer_size_x << " "
                          << halo.buffer_size_z << std::endl;
            ...
            if (RemeltingYN)
                ...
                CellTypeInit_NoRemelt(layernumber + 1, halo, ZBound_Low, LocalActiveDomainSize, CellType, CritTimeStep,
                                      NeighborX, NeighborY, NeighborZ, NGrainOrientations, GrainUnitVector,
                                      DiagonalLength, GrainID, CritDiagonalLength, DOCenter, LayerID);
    ...
    if (((PrintMisorientation) || (PrintFinalUndercoolingVals) || (PrintFullOutput)) || (PrintDefaultRVE)) {
        ...
        // Host mirrors of CellType and GrainID are not maintained - pass device views and perform copy inside of
        // subroutine
        PrintExaCAData(NumberOfLayers - 1, halo, GrainID, CritTimeStep, GrainUnitVector, LayerID, CellType,
                       UndercoolingChange, UndercoolingCurrent, OutputFile, NGrainOrientations, PathToOutput, 0,
                       PrintMisorientation, PrintFinalUndercoolingVals, PrintFullOutput, false, PrintDefaultRVE, 0,
                       ZBound_Low, nzActive, deltax, XMin, YMin, ZMin, NumberOfLayers, PrintBinary, RVESize);
    }
    else {
        if (id == 0)
